...
def decrypt(c, p, q):
  m1 = p * ( pow(p,q-2,q) * pow(c,(q+1)/4,q) )
  m2 = q * ( pow(q,p-2,p) * pow(c,(p+1)/4,p) )

  ma = (m1 + m2) % (nrabin)
  if m1 > m2: 
     mb = (m1 - m2) % (nrabin)
  else:
     mb = (nrabin + (m1 - m2)) % (nrabin)
  s_ = 2**500
  return [num2code(ma/s_), num2code(mb/s_), num2code((nrabin - ma)/s_), num2code((nrabin - mb)/s_)] 
...
if len(sys.argv) == 3 and sys.argv[1] == "E":
  print " digital signature:\n " + num2code( (2**500 * code2num(sys.argv[2]))**2 % nrabin)

if len(sys.argv) == 3 and sys.argv[1] == "D":
  print " digital signature:\n " + str(decrypt(code2num(sys.argv[2]), readNumber("p_"), readNumber("q_")))
